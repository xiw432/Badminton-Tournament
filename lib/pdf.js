...
export async function generateAdmitCardPDF(player) {
  ...
  page.drawText('Dates: April 22-24, 2026   |   Venue: Gopi Nath Laxman Das Rastogi Inter College, Aishbagh, Lucknow', {
  ...
  events.forEach(ev => {
    ...
    const partnerName = typeof ev === 'object' ? ev.partnerName : null;
    page.drawText(`• ${name}`, { x: 50, y, size: 10, font: reg, color: BLACK });
    y -= 16;
    if (partnerName) {
      page.drawText(`  Partner: ${partnerName}`, { x: 60, y, size: 9, font: reg, color: rgb(0.02, 0.37, 0.02) });
      y -= 14;
    }
  });
